...
from tensor2tensor.common_attention import multihead_attention, add_timing_signal_1d, attention_bias_ignore_padding, attention_bias_lower_triangle, attention_bias_upper_triangle
...
class Model(object):
    ...
    def build_variational_train_model(self):
        ...
        with self.graph.as_default():
            ...
            self.gan_train_op_ab = self.gan_optimizer.apply_gradients(gan_grads_and_vars_ab, global_step=self.global_step)
            ...
            self.gan_train_op_ba = self.gan_optimizer.apply_gradients(gan_grads_and_vars_ba, global_step=self.global_step)
    ...
    def variational_encoder(self, input_x, layers_indep, layers_share, vocab_size, scope="encoder", reuse=None, step='f'):
        ...
        bias_ignore_padding = attention_bias_ignore_padding(encoder_padding)
        bias_forward_padding = attention_bias_lower_triangle(tf.shape(encoder_padding)[1])
        bias_backward_padding = attention_bias_upper_triangle(tf.shape(encoder_padding)[1])
        ...
        with tf.variable_scope(scope, initializer=self._initializer, reuse=reuse):
            ...
            encoder_output=self.build_enc_block(encoder_output, encoder_padding, bias_forward_padding, 1,
                                                scope+'indep_block_forward_1', reuse=reuse)

            encoder_output=self.build_enc_block(encoder_output, encoder_padding, bias_backward_padding, 1,
                                                scope+'indep_block_backward_1', reuse=reuse)

            encoder_output=self.build_enc_block(encoder_output, encoder_padding, bias_forward_padding, 1,
                                                scope+'indep_block_forward_2', reuse=reuse)

            encoder_output=self.build_enc_block(encoder_output, encoder_padding, bias_backward_padding, 1,
                                                scope+'indep_block_backward_2', reuse=reuse)

        encoder_output=self.build_enc_block(encoder_output, encoder_padding, bias_ignore_padding, layers_share,
    ...
    def build_enc_block(self, input_x, encoder_padding, encoder_padding_bias, layers, scope, reuse=False):
        encoder_padding=encoder_padding
        encoder_padding_bias = encoder_padding_bias
