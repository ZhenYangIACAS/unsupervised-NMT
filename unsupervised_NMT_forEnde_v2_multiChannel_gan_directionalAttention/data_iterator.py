...
class unSuperGanTextIterator:
    def __init__(self, s_domain_data, t_domain_data, s_domain_generated_data, t_domain_generated_data, dic_s, dic_t, batch=1, maxlen=50, n_words_source=-1, n_words_target=-1):
         self.s_domain_data = fopen(s_domain_data, 'r')
         self.s_domain_generated_data = fopen(s_domain_generated_data, 'r')
         self.t_domain_data = fopen(t_domain_data, 'r')
         self.t_domain_generated_data = fopen(t_domain_generated_data, 'r')

         with open(dic_t) as f_trg, open(dic_s) as s_trg:
    ...
    def reset(self):
         ...
         self.s_domain_generated_data.seek(0)
         self.t_domain_data.seek(0)
         self.t_domain_generated_data.seek(0)
        ...
        try:
            while True:
                ...
                if ss == "":
                    raise IOError
                ss = ss.strip().split()
                ss = [self.dic_source[w] if w in self.dic_source else 1 for w in ss]
                if self.n_words_src > 0:
                    ss = [w if w < self.n_words_src else 1 for w in ss]
                ...
                sg = self.s_domain_generated_data.readline()
                if sg == "":
                    raise IOError
                sg = sg.strip().split()
                sg = [self.dic_source[w] if w in self.dic_source else 1 for w in sg]
                if self.n_words_src > 0:
                    sg = [w if w < self.n_words_src else 1 for w in sg]

                tg = self.t_domain_generated_data.readline()
                if tg == "":
                    raise IOError
                tg = tg.strip().split()
                tg = [self.dic_target[w] if w in self.dic_target else 1 for w in tg]
                if self.n_words_trg > 0:
                    tg = [w if w < self.n_words_trg else 1 for w in tg]

                if len(ss) > self.maxlen or len(tt) >self.maxlen or len(sg) > self.maxlen or len(tg) > self.maxlen:
                    continue
